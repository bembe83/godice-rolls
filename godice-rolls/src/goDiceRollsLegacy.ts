...
export default class goDiceRollsLegacy {
	static PREF_USE_LEGACY = 'use-legacy';

	static get useLegacy(): boolean { return SETTINGS.get(goDiceRollsLegacy.PREF_USE_LEGACY); }
	...
	static patch() {
		...
		Handlebars.registerHelper("itemDamage", (item, rollData) => {
			...
			const reduceFormula = (formula: any) => {
				...
				let roll: any;
				try {
					goDiceRolls.tempDisable = true;
					roll = RollPF.safeRoll(formula, rollData);
				} finally {
					goDiceRolls.tempDisable = false;
				}
		...
		if (this.modifiers.length == 0) {
			...
			for (let n = 1; n <= this.number; n++) {
				...
				if (minimize) {
					...
					const result = goDiceRollsLegacy.prompt(1, this.faces, this.flavor);
					roll.result = result[0];
					flags.push(result[1] ? 'MR' : 'RN');
				}
				this.results.push(roll);
			}
			if (goDiceRolls.flagged && flags.some(x => x === 'MR')) {

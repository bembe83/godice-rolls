...
export default class goDiceRolls {
	...
	static get flagged(): boolean { return SETTINGS.get(goDiceRolls.PREF_FLAGGED); }
	static get toggled(): boolean { return SETTINGS.get(goDiceRolls.PREF_TOGGLED); }
	static setToggled(value: boolean): Promise<boolean> { return SETTINGS.set(goDiceRolls.PREF_TOGGLED, value); }
	static get toggleable() {
		return (game.user.getFlag(SETTINGS.MOD_NAME, goDiceRolls.FLAG_ROLL_TYPE) || SETTINGS.get(game.user.isGM ? goDiceRolls.PREF_GM_STATE : goDiceRolls.PREF_PC_STATE)) === 'toggle';
	}
	static tempDisable = false;
	static get shouldRollManually() {
		const state = game.user.getFlag(SETTINGS.MOD_NAME, goDiceRolls.FLAG_ROLL_TYPE) || SETTINGS.get(game.user.isGM ? goDiceRolls.PREF_GM_STATE : goDiceRolls.PREF_PC_STATE);
	...
	private static async _Roll_evaluate(this: Roll, wrapper: (arg: any) => any, { minimize = false, maximize = false } = {}): Promise<Roll> {
		...
		if (!goDiceRolls.shouldRollManually || minimize || maximize) {
	...
	private static async _DiceTerm_evaluate(this: DiceTerm, wrapper: AnyFunction, { minimize = false, maximize = false } = {}): Promise<DiceTerm> {
		...
		if (!goDiceRolls.shouldRollManually || !rollPrompt || minimize || maximize)
			return wrapper(minimize, maximize);
